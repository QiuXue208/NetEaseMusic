        ...
        setInterval(()=>{
            ...
            for(let i=0; i<$lines.length; i++){
                ...
                if($lines[i+1] !== undefined && $currentLineTime <= time && $nextLineTime > time){
                    let height = $lines[i].offset.top - 
                    $lines.css('transform',`translateY()`)
                    console.log($lines[i])
                }
            }
        },1000)
...
$(function () {
    ...
    $.get('./songs.json').then(function (response) {
        let songs = response
        let song = songs.filter(function(value,key){
